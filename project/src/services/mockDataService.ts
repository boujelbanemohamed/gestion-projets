...
export class MockDataService {
  ...
  // PV de réunion
  async getMeetingMinutes(projectId?: string): Promise<{ meetingMinutes: any[] }> {
    let meetings = this.data.meeting_minutes || [];

    // Filtrer par projet si spécifié
    if (projectId) {
      meetings = meetings.filter(meeting =>
        meeting.project_id?.toString() === projectId.toString()
      );
    }

    // Retourner dans le format attendu par les composants
    return { meetingMinutes: meetings };
  }
