-- CORRECTION FINALE DES DOUBLONS D'EMAILS - AVEC GESTION DES REFERENCES
-- Ce script corrige tous les doublons en gerant d'abord les references FK
...
-- ========================================
-- 3. ANALYSE DES REFERENCES
-- ========================================

-- Voir les projets qui referencent des utilisateurs avec IDs differents
SELECT '=== PROJETS AVEC REFERENCES PROBLEMATIQUES ===' as info;
SELECT 
  p.id as projet_id,
  p.nom as projet_nom,
  p.created_by as old_user_id,
  au.id as auth_user_id,
  au.email,
  CASE 
    WHEN au.id = p.created_by THEN 'ID CORRESPOND'
    ELSE 'ID DIFFERENT - CORRECTION NECESSAIRE'
  END as statut
FROM public.projects p
INNER JOIN public.users pu ON p.created_by = pu.id
INNER JOIN auth.users au ON pu.email = au.email
WHERE au.id != pu.id;

-- Voir les taches qui referencent des utilisateurs avec IDs differents
SELECT '=== TACHES AVEC REFERENCES PROBLEMATIQUES ===' as info;
SELECT 
  t.id as tache_id,
  t.titre as tache_titre,
  t.assigned_to as old_user_id,
  au.id as auth_user_id,
  au.email,
  CASE 
    WHEN au.id = t.assigned_to THEN 'ID CORRESPOND'
    ELSE 'ID DIFFERENT - CORRECTION NECESSAIRE'
  END as statut
FROM public.tasks t
INNER JOIN public.users pu ON t.assigned_to = pu.id
INNER JOIN auth.users au ON pu.email = au.email
WHERE au.id != pu.id;

-- ========================================
-- 4. CORRECTION DES REFERENCES
-- ========================================
...
-- ========================================
-- 5. SUPPRESSION DES ENREGISTREMENTS PROBLEMATIQUES
-- ========================================
...
-- ========================================
-- 6. CREATION DES PROFILS AVEC LES BONS IDS
-- ========================================

-- Creer les profils manquants pour les utilisateurs auth
SELECT '=== CREATION PROFILS AVEC BONS IDS ===' as info;
