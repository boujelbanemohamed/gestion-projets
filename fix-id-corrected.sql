-- Correction corrigee de l'ID pour mohamed.boujelbane@icloud.com
-- Ce script corrige l'incoherence en creant d'abord le bon profil
...
-- 4. Creer d'abord le profil avec le bon ID (celui de auth.users)
SELECT 'CREATION DU PROFIL AVEC LE BON ID' as info;
...
-- 5. Maintenant mettre a jour les references vers le bon ID
SELECT 'MISE A JOUR DES REFERENCES VERS LE BON ID' as info;
...
WHERE created_by = (SELECT id FROM public.users WHERE email = 'mohamed.boujelbane@icloud.com' AND id != (SELECT id FROM auth.users WHERE email = 'mohamed.boujelbane@icloud.com'));
...
WHERE assigned_to = (SELECT id FROM public.users WHERE email = 'mohamed.boujelbane@icloud.com' AND id != (SELECT id FROM auth.users WHERE email = 'mohamed.boujelbane@icloud.com'));
...
-- 6. Maintenant on peut supprimer l'ancien enregistrement en toute securite
